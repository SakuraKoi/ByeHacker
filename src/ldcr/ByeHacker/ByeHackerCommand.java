				...
				if ("ByeHacker-AutoDetect".equals(banInfo.getSource())) {
					if (banInfo.getReason().startsWith("ByeHacker-Detected| ")) {
						...
						sender.sendMessage(new String[] {
								"§b§lByeHacker §7>> §a玩家 "+player.getName()+" 被检测于 "+DateUtils.formatDate(banInfo.getCreated()),
								"§b§lByeHacker §7>> §a 所在IP: "+infos[1]
								});
					} else {
						sender.sendMessage("§b§lByeHacker §7>> §a玩家 "+player.getName()+" 被旧版ByeHacker检测封禁, 无法查询信息");
					}
				} else {
			...
			if (Bukkit.getBanList(Type.NAME).isBanned(player.getName())) {
				...
				if ("ByeHacker-AutoDetect".equals(banInfo.getSource())) {
					if (banInfo.getReason().startsWith("ByeHacker-Detected| ")) {
						...
						sender.sendMessage("§b§lByeHacker §7>> §a已解封玩家 "+playerName+" ["+infos[1]+"]");
					} else {
						sender.sendMessage("§b§lByeHacker §7>> §a玩家 "+player.getName()+" 被旧版ByeHacker检测封禁, 无法解封");
					}
				} else {
					sender.sendMessage("§b§lByeHacker §7>> §a玩家 "+player.getName()+" 不是被ByeHacker检测封禁的");
				}
